...
def test_vault_shutdown_can_withdraw(
    ...
    # transfer all the left token and set the balance zero
    ...
    chain.sleep(3600 * 7)
    chain.mine(1)
    assert strategy.estimatedTotalAssets() >= amount - 2  # minus 2 due to roundig error
...
def test_basic_shutdown(
    ...
    assert token.balanceOf(vault) >= amount - 2  # The vault has all funds
